...
{
	CP3 p[4];
	double theta, phi; // theta控制小圆旋转，phi控制大圆旋转
	double pi = 3.14159, delta = pi / 20;
	double R = 150; // 大圆半径
	double r = 50;	// 小圆半径
	...
	for (theta = 0; theta <= 2 * pi; theta += delta)
		...
		{
			// 计算圆环体上的四个顶点
			p[0].x = (R + r * cos(theta)) * cos(phi);
			p[0].y = (R + r * cos(theta)) * sin(phi);
			p[0].z = r * sin(theta);

			p[1].x = (R + r * cos(theta)) * cos(phi + delta);
			p[1].y = (R + r * cos(theta)) * sin(phi + delta);
			p[1].z = r * sin(theta);

			p[2].x = (R + r * cos(theta + delta)) * cos(phi + delta);
			p[2].y = (R + r * cos(theta + delta)) * sin(phi + delta);
			p[2].z = r * sin(theta + delta);

			p[3].x = (R + r * cos(theta + delta)) * cos(phi);
			...
			CVector ViewVector(p[0], ViewPoint);
			ViewVector = ViewVector.Normalize();
			CFace f;
			f.SetFaceNormal(p[0], p[1], p[2]);
			f.fNormal.Normalize();
